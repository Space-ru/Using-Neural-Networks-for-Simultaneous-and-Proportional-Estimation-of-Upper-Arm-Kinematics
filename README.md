...
The code and data are organized per subject (S1 to S5) with the code being repeated per subject and neural network. The pipeline is as follows:
1. Extract_EMG_position.m: from the raw data (unavailable) extract EMG data and VICON positions. The joint angle is calculated and all data is stored in TestTrial.mat files (one file per trial).
