...
%   
% 	Description: MLP/Linear Regression Preprocessing - Combined program
% 	which includes finding and setting the normalisation values. This file
% 	is run two times, first for two trials to find the normalisation
% 	values, then to process the EMG signal.
%   
...
for trialnumber = 1:8% adjust accordingly
...
% Plot bicep features
 figure; 
    ...
    xlabel('time (s)');
    ylabel('EMG Voltage');
    title('Processing A.Deltoid EMG data');
...
% Plot tricep signal's features
 ...
 legend('Raw EMG','RMS','Location','northoutside','Orientation','horizontal');
    xlabel('time (s)');
    ylabel('EMG Voltage');
    title('Processing EMG P.Deltoid data');
    index = index + 1;
%% Store into excel 
